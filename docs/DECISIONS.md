...
- **Chatto Metro canonical address** — "Bastuhara, Khetchar, Behind Noman College, Left of Shah Amanat Bridge Junction, Chattogram". Used in `event.ts` EventEntry location, JSON-LD streetAddress, and any copy. Do not shorten.
...
## Locked Decisions (added 2026-07-09)

- **Chattogram Duathlon 2026 gets its own decorated page + skin** — `/events/duathlon-2026` (branch inside `[slug].astro`, `eventEntry.slug === 'duathlon-2026'`, not a separate route file). Chartreuse `#C6F53F` on the shared `--bg-base`, a bike-chainring motif replacing the ship's wheel, diagonal-split compositions, Run-Bike-Run strip. Fully scoped under `.dua-page` / `dua-*` classes in `global.css` — see `docs/CLAUDE.md` Key Constraints. Content lives in new `dua*`-prefixed exports in `event.ts`, mirroring the existing CMHM export shape.
- **Homepage next-race feature superseded** — `TwoRegisterCTAs.astro` + `UpcomingEventsList.astro` deleted; replaced by `DualEventFeature.astro` (two feature cards — duathlon chartreuse/open, Chatto Metro gold/rescheduled-closed — plus an "Also on the calendar" strip built from `getUpcomingEvents()` filtered to exclude the two featured events). Homepage wheel-divider count dropped from 3 to 2 (`TRACK RECORD`, `THE MISSION`) — no divider between hero and the dual-event section.
- **`BrandHero.astro` rewritten** — no longer shows a current-event card + countdown ring; now a 2-column `tb-hero` (mission copy left, wide `triathlon-bd-shield-white.png` logo right) matching the dual-event homepage. The event countdown lives on the duathlon page hero (`ChainringCountdown`) and, as static day-counts, on the `DualEventFeature` cards.
